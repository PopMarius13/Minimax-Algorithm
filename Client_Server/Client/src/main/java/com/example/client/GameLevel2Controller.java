...
public class GameLevel2Controller {
    ...
    public void initialize() {
        Client.getINSTANCE().sendMessage("Arrow Number:" + arrowNumber);
            ...
            button.setOnAction(new EventHandler<ActionEvent>() {
                ...
                public void handle(ActionEvent actionEvent) {
                    ...
                    if (buttonsCheck[(x - 1) * 8 + y] != 1) {
                        ...
                        Client.getINSTANCE().sendMessage("Player Move:" + (x - 1) + (y - 1) + arrow);
                        int finishGame = Integer.parseInt(Client.getINSTANCE().sendMessage("Finish Game:true"));
                        if (finishGame != 0) {
                            finishGame(finishGame);
                        } else {
                            String[] pointNew = Client.getINSTANCE().sendMessage("Computer Move").split(",");
                            int px = Integer.parseInt(pointNew[0]);
                            int py = Integer.parseInt(pointNew[1]);
                            String urlImage = "image/arrow" + Integer.parseInt(pointNew[2]) + "c.png";
                            BackgroundImage bimg = new BackgroundImage(new Image(urlImage),
                                    BackgroundRepeat.NO_REPEAT,
                                    BackgroundRepeat.NO_REPEAT,
                                    BackgroundPosition.CENTER,
                                    new BackgroundSize(52, 52, false, false, false, false));
                            buttons[px * 4 + py + 1].setBackground(new Background(bimg));
                            finishGame =  Integer.parseInt(Client.getINSTANCE().sendMessage("Finish Game:true"));
                            if (finishGame != 0) {
                                finishGame(finishGame);
                            }
                        }
